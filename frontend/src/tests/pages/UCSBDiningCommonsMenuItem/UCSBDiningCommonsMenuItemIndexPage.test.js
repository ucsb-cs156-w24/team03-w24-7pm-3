...
describe("UCSBDiningCommonsMenuItemIndexPage tests", () => {
    ...
    test("Renders with Create Button for admin user", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText(/Create Dining Commons Menu Item/)).toBeInTheDocument();
        });
    ...
    test("renders three ucsb dining commons menu items correctly for regular user", async () => {
        setupUserOnly();
        axiosMock.onGet("/api/ucsbdiningcommonsmenuitem/all").reply(200, ucsbDiningCommonsMenuItemFixtures.threeRestaurants);
        ...
        const createDiningCommonsMenuItem = screen.queryByText("Create Dining Commons Menu Item");
        expect(createDiningCommonsMenuItem).not.toBeInTheDocument();

        const diningCommonsCode = screen.getByText("ORTG");
        expect(diningCommonsCode).toBeInTheDocument();
        
        const name = screen.getByText("Breakfast Burrito");
        expect(name).toBeInTheDocument();

        const station = screen.getByText("Main Station");
